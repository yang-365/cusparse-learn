...
int main(){
  ...
  /* exsercise conversion routines (convert matrix from COO 2 CSR format) */
  ...
  /* excersise Level 1 routines (scatter vector elements) */
  ...
  /* excercise Level 2 routines (csrmv) */
  status = cusparseDcsrmv(handle,CUSPARSE_OPERATION_NON_TRANSPOSE,n,n,nnz,&dtwo,descr,cooVal,csrRowPtr,cooColIndex,&y[0],&dthree,&y[n]);
  if(status != CUSPARSE_STATUS_SUCCESS){
  ...
  /* excercise Level 3 routines (csrmm) */
  ...
  status = cusparseDcsrmm(handle, CUSPARSE_OPERATION_NON_TRANSPOSE,n,2,n,nnz,&dfive,descr,cooVal,csrRowPtr,cooColIndex,y,n,&dzero,z,n+1);
  if(status != CUSPARSE_STATUS_SUCCESS){
  ...
  // z = [950 400 2550 2600 0 | 49300 12500 132300 131200 0]
  ...
  handle = 0;
  if(status != CUSPARSE_STATUS_SUCCESS){
    CLEANUP("CUSPARSE Library relsease of resource failed");
    return 1;
  }
